...
final class LazySuppressionReplacement extends Replacement {
    ...
    public Range<Integer> range() {
        // If we're removing everything and have source code, extend range to include preceding whitespace
        String replacementText = calculateReplacementText();
        if (replacementText.isEmpty() && suppressWarnings.isPresent() && sourceCode.isPresent()) {
            DiagnosticPosition position = (DiagnosticPosition) suppressWarnings.get();
            int extendedStart = SourceCodeUtils.startPositionWithWhitespaceIncludingNewLine(
                    sourceCode.get(), position.getStartPosition());
            return Range.closedOpen(extendedStart, range.upperEndpoint());
        }
        return range;
    }
    ...
    public String replaceWith() {
        return calculateReplacementText();
    }
